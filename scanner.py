# ==============================================================================
# FAROS v22.0 - THE MASTER SUITE (FULL STACK INTEGRATION)
# Autor: Juan Arroyo | SG Consulting Group & Emporium
# Módulos: AI Chatbot, Macro, Portfolio (TAI), Scanner, Backtest, Oracle
# ==============================================================================
...
def get_ecuador_time():
    """Hora actual en Ecuador (UTC-5) para reportes."""
...
def generate_faros_insight(ticker, risk_tolerance=3.0):
    try:
        ...
        if z_entropy > risk_tolerance:
            state = "🔴 **FASE GASEOSA (Caos)**"; state_desc = f"Alta entropía ({z_entropy:.1f}σ). Estructura inestable."
        elif trend > 0.05:
            state = "🟢 **FASE LÍQUIDA (Flujo)**"; state_desc = f"Tendencia fuerte (+{trend*100:.1f}%) con volatilidad controlada."
        elif trend < -0.05:
            state = "🧊 **FASE SÓLIDA (Ruptura)**"; state_desc = "Precio congelado bajo su estructura media."
        else:
            state = "🟡 **FASE PLASMA**"; state_desc = "Transición lateral o ilíquida."

        verdict = "Oportunidad Institucional" if psi > 70 else ("Activo Especulativo" if psi > 40 else "Activo Tóxico/Riesgoso")

        return f"""
        ### 📡 Análisis TAI: {ticker}
        **Precio:** ${curr_price:.2f} | **Score Ψ:** {psi:.0f}/100
        **Diagnóstico:** {state}
        > {state_desc}
        **Veredicto:** {verdict}
        *(Volatilidad: {raw_vol:.1f}% | Tendencia: {trend*100:+.1f}%)*
        """
    except: return "Error analizando el activo."
...
def calculate_tai_weights(tickers, risk_tolerance):
    ...
    for t in tickers:
        try:
            ...
            if len(hist) > 50:
                _, z_entropy = calculate_entropy(hist); sma = hist['Close'].rolling(50).mean().iloc[-1]; trend = (hist['Close'].iloc[-1] - sma) / sma
                psi = calculate_psi(z_entropy, 0, trend, risk_tolerance, global_penalty)
...
# ==============================================================================
# 3. FUNCIONES DE ANÁLISIS (MÓDULOS)
# ==============================================================================
...
# ==============================================================================
# 5. INTERFAZ DE USUARIO (FRONT-END PRINCIPAL)
# ==============================================================================

with st.sidebar:
    st.title("📡 FAROS")
    st.caption("**By SG Consulting Group y Emporium**")
    app_mode = st.radio("SISTEMA:", ["🤖 ANALISTA IA", "🌎 MACRO ECONOMÍA", "💼 GESTIÓN PORTAFOLIOS", "🔍 SCANNER MERCADO", "⏳ BACKTEST LAB", "🔮 ORÁCULO FUTURO"])
    st.markdown("---")
...
if app_mode == "🤖 ANALISTA IA":
    ...
    st.caption("Consulta el estado termodinámico de cualquier activo.")
    ...
    if prompt := st.chat_input("Escribe tu consulta aquí..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Procesando datos en tiempo real..."):
                ...
                response = generate_faros_insight(ticker, risk_sigma) if ticker else "No detecté un ticker específico. Intenta mencionar el símbolo (ej: NVDA)."
...
# --- MÓDULO 2: MACRO ---
    ...
    if st.button("Escanear Economía"):
        with st.spinner(f"Analizando {country_sel}..."):
...
# --- MÓDULO 4: SCANNER ---
    ...
    if st.button("Analizar Mercado"): 
        ...
        st.session_state['scan_data'] = df; st.session_state['scan_meta'] = (m_status, m_entropy, m_msg)
    
    if 'scan_data' in st.session_state:
        df = st.session_state['scan_data']; m_status, m_entropy, m_msg = st.session_state['scan_meta']
